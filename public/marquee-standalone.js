(() => {
  ...
  function renderError(message, retry = false) {
    ensureStyle();
    app.innerHTML = `<main class="mq-shell"><div class="mq-error"><div>${esc(message || '載入失敗')}</div>${retry ? '<button type="button" class="mq-btn" data-retry-marquee>重新載入</button>' : ''}</div></main>`;
    if (retry) app.querySelector('[data-retry-marquee]')?.addEventListener('click', () => start());
  }
  ...
  async function fetchMarqueeConfig() {
    let lastError = null;
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      try {
        const { response, result } = await fetchJson(`/api/marquee?_=${Date.now()}_${attempt}`, {}, 2500);
        if (response.ok && result.success) return result;
        lastError = new Error(result.message || `HTTP ${response.status}`);
      } catch (error) {
        lastError = error;
      }
      if (attempt < 3) await wait(180 * attempt);
    }
    throw lastError || new Error('廣告資料讀取失敗');
  }
  ...
  async function runStart() {
    ...
    let result;
    try {
      result = await fetchMarqueeConfig();
    } catch {
      renderError('廣告資料連線逾時，請重新載入。', true);
      return;
    }
    ...
    app.querySelectorAll('.mq-slide').forEach((button)=>button.addEventListener('click', async()=>{
      ...
      try {
        ...
        const { response: reward, result: data } = await fetchJson('/api/marquee/reward',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({lineUserId:uid,imageId:item.id||'',imageUrl:item.imageUrl||''})},5000);
      ...
      } catch(error){ renderError(error?.message||'贈點失敗', true); }
    }));
    ...
    pointsBtn?.addEventListener('click', async()=>{
      ...
      try {
        ...
        const uid=await requireUid();
        const { response: r, result: data } = await fetchJson('/api/marquee/points',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({lineUserId:uid})},5000);
  ...
  function start() {
    ...
    startPromise = Promise.resolve(runStart()).finally(() => { startPromise = null; });
    return startPromise;
  }
