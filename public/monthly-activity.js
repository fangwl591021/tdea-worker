(() => {
  ...
  function blankConfig() {
    return { enabled: true, keyword: "TDEA每月活動", month: new Date().toISOString().slice(0, 7), altText: "TDEA 每月活動", pages: [blankPage()] };
  }
  ...
  function adminEmail() {
    let email = sessionStorage.getItem(adminKey) || "";
    if (!email) {
      email = prompt("請輸入管理者 Email") || "";
      if (email) sessionStorage.setItem(adminKey, email);
    }
    return email;
  }
  ...
  function render() {
    ...
    main.innerHTML = `
      <div class="topbar"><div><h1>每月活動</h1><div class="subtitle">固定產出 TDEA 每月活動 carousel，發布後關鍵字 TDEA每月活動 會啟用。</div></div><div class="actions"><button class="btn" data-monthly-json>複製 FLEX JSON</button><button class="btn primary" data-monthly-publish>發布</button></div></div>
      <div class="monthly-workspace"><div class="monthly-left">
        <section class="panel"><div class="panel-head"><h2 class="panel-title">基本設定</h2></div><div class="monthly-form">${basicFields()}</div></section>
        <section class="panel"><div class="panel-head"><h2 class="panel-title">活動頁數</h2><div class="actions"><button class="btn" data-monthly-add>新增頁</button><button class="btn danger" data-monthly-delete>刪除頁</button></div></div><div class="monthly-pages">${config.pages.map((item, index) => `<button class="monthly-page-btn ${index === selected ? "active" : ""}" data-monthly-select="${index}">第 ${index + 1} 頁<div class="muted">${esc(item.detailTitle || item.formUrl || "未命名")}</div></button>`).join("")}</div></section>
        <section class="panel"><div class="panel-head"><h2 class="panel-title">第 ${selected + 1} 頁設定</h2></div>${pageForm(page)}</section>
      </div><aside class="panel monthly-preview-panel"><div class="panel-head"><h2 class="panel-title">預覽區</h2><span class="muted">橫式多頁 FLEX</span></div><div style="padding:18px">${preview()}</div></aside></div><div class="toast" id="monthly-toast"></div>`;
    bind();
  }
  ...
  function bind() {
    document.querySelectorAll("[data-monthly-basic]").forEach((input) => input.addEventListener("input", () => { config[input.name] = input.value; render(); }));
    ...
    document.querySelectorAll("[data-monthly-page]").forEach((input) => input.addEventListener("input", () => { config.pages[selected][input.name] = input.value; render(); }));
  ...
  async function uploadImage(event) {
    ...
    if (!email) return toast("需要管理者 Email 才能上傳");
  ...
  async function publish() {
    const email = adminEmail();
    if (!email) return toast("需要管理者 Email 才能發布");
    config.keyword = "TDEA每月活動";
