(() => {
  ...
  function adminEmail() {
    ...
    if (!email) { email = prompt("請輸入管理者 Email") || ""; if (email) sessionStorage.setItem(adminKey, email); }
    return email;
  }
  ...
  async function uploadImage(event) {
    ...
    const email = adminEmail(); if (!email) return toast("需要管理者 Email 才能上傳");
  ...
  async function publish() {
    const email = adminEmail(); if (!email) return toast("需要管理者 Email 才能發布");
