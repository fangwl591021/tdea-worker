...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if ((request.method === "POST" || request.method === "GET") && url.pathname === "/api/aiwe-sync-roster") {
      try {
        return await syncRosterMembers(request, env);
      } catch (error) {
        return json({ success: false, message: String((error as Error).message || error), stack: String((error as Error).stack || "").slice(0, 800) }, 500);
      }
    }
